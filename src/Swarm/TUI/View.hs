  ...
  , joinBorders $ hBox
    ...
    , vBox
      ...
      , panel
        ...
        &  topLabels
        .  rightLabel
        .~ (drawType <$> (s ^. uiState . uiReplType))
        )
        (vLimit replHeight $ padBottom Max $ padLeftRight 1 $ drawREPL s)
      ]
    ]
  ]
  where fr = s ^. uiState . uiFocusRing
...
renderModal modal = renderDialog (dialog (Just modalTitle) Nothing 500)
                                 modalWidget
 where
  ...
  (modalTitle, modalContent) = case modal of
...
helpWidget = (helpKeys <=> fill ' ') <+> (helpCommands <=> fill ' ')
 where
  helpKeys =
    vBox [hCenter $ txt "Global Keybindings", hCenter $ mkTable glKeyBindings]
  ...
  helpCommands =
    vBox [hCenter $ txt "Commands", hCenter $ mkTable baseCommands]
  baseCommands =
...
drawMenu isPaused viewingBase mode =
 ...
 where
  gameModeWidget =
    padLeft Max . padLeftRight 1 . txt . (<> " mode") $ case mode of
      Classic  -> "Classic"
      Creative -> "Creative"
  globalKeyCmds = [("F1", "help"), ("Tab", "cycle panels")]
  keyCmdsFor (Just REPLPanel) = [("Enter", "execute")]
  ...
  keyCmdsFor (Just InfoPanel) =
    [("↓↑/Pg{Up,Dn}/Home/End", "navigate"), ("Enter", "make")]
  keyCmdsFor _ = []
...
drawWorld g =
 ...
 where
  ...
  robotsByLoc =
    M.fromListWith (maxOn (^. robotDisplay . displayPriority))
      .  map (view robotLocation &&& id)
      .  M.elems
      $  g
      ^. robotMap
  drawLoc coords = case M.lookup (W.coordsToLoc coords) robotsByLoc of
    Just r -> withAttr (r ^. robotDisplay . displayAttr) $ str
      [ lookupDisplay ((r ^. robotOrientation) >>= toDirection)
                      (r ^. robotDisplay)
      ]
...
drawRobotInfo s =
  case (s ^. gameState . to focusedRobot, s ^. uiState . uiInventory) of
    (Just r, Just (_, lst)) ->
      let V2 x y = r ^. robotLocation
      in  padBottom Max $ vBox
            [ hCenter $ hBox
              [ txt (r ^. robotName)
            ...
            , padAll
              1
              (BL.renderListWithIndex (drawItem (lst ^. BL.listSelectedL))
                                      isFocused
                                      lst
              )
            ]
 ...
 where
  isFocused =
    (s ^. uiState . uiFocusRing . to focusGetCurrent) == Just InfoPanel
...
drawItem _ _ _ (InventoryEntry n e) = drawLabelledEntityName e <+> showCount n
  where showCount = padLeft Max . str . show
...
drawLabelledEntityName e =
  hBox [padRight (Pad 2) (displayEntity e), txt (e ^. entityName)]
