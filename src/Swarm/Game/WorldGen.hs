...
testWorld1 (Coords (i, j))
  | noiseValue pn1 (fromIntegral i, fromIntegral j, 0) > 0
  = (DirtT, Just "tree")
  | noiseValue pn2 (fromIntegral i, fromIntegral j, 0) > 0
  = (StoneT, Just "rock")
  | otherwise
  = (GrassT, Nothing)
 where
...
testWorld2 (Coords ix@(r, c)) = genBiome
 ...
 where
  ...
  genBiome Big Hard Natural | sample ix cl0 > 0.7 = (StoneT, Just "mountain")
                            | h `mod` 30 == 0     = (StoneT, Just "boulder")
                            | sample ix cl0 > 0   = (DirtT, Just "tree")
                            | otherwise           = (GrassT, Nothing)
  genBiome Small Hard Natural | h `mod` 10 == 0 = (StoneT, Just "rock")
                              | otherwise       = (StoneT, Nothing)
  genBiome Big Soft Natural | even (r + c) = (WaterT, Just "wave")
                            | otherwise    = (WaterT, Nothing)
  genBiome Small Soft Natural | h `mod` 10 == 0 = (GrassT, Just "flower")
                              | otherwise       = (GrassT, Nothing)
  genBiome Small Soft Artificial
    | h `mod` 10 == 0
    = (GrassT, Just (T.concat ["bit (", from (show ((r + c) `mod` 2)), ")"]))
    | otherwise
    = (GrassT, Nothing)
  genBiome Big Soft Artificial | h `mod` 5000 == 0   = (DirtT, Just "linux")
                               | sample ix cl0 > 0.5 = (GrassT, Nothing)
                               | otherwise           = (DirtT, Nothing)
  genBiome Small Hard Artificial | h `mod` 120 == 1 = (StoneT, Just "lambda")
                                 | otherwise        = (StoneT, Nothing)
  ...
  sample (i, j) noise =
    noiseValue noise (fromIntegral i / 2, fromIntegral j / 2, 0)
...
findGoodOrigin f = \(Coords (r, c)) -> f (Coords (r + rOffset, c + cOffset))
 where
  ...
  (rOffset, cOffset) =
    fromMaybe (error "the impossible happened, no offsets were found") offsets
