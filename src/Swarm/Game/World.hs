...
tileBounds =
  (TileOffset (Coords (0, 0)), TileOffset (Coords (tileMask, tileMask)))
...
plusOffset (Coords (x1, y1)) (TileOffset (Coords (x2, y2))) =
  Coords (x1 `xor` x2, y1 `xor` y2)
...
lookupTerrain i (World f t _) =
  ((U.! tileOffset i) . fst <$> M.lookup (tileCoords i) t) ? fst (f i)
...
lookupTerrainM
  :: (MonadState (World t e) m, IArray U.UArray t) => Coords -> m t
lookupTerrainM c = do
  modify $ loadCell c
  lookupTerrain c <$> get
...
lookupEntity i (World f t m) =
  M.lookup i m ? ((A.! tileOffset i) . snd <$> M.lookup (tileCoords i) t) ? snd
    (f i)
...
--   then looks up the terrain value.
lookupEntityM
  :: (MonadState (World t e) m, IArray U.UArray t) => Coords -> m (Maybe e)
lookupEntityM c = do
  modify $ loadCell c
  lookupEntity c <$> get
...
update i g w@(World f t m) = World f t (M.insert i (g (lookupEntity i w)) m)

-- | A stateful variant of 'update', which also ensures the tile
--   containing the given coordinates is loaded.
updateM
  :: (MonadState (World t e) m, IArray U.UArray t)
  => Coords
  -> (Maybe e -> Maybe e)
  -> m ()
...
loadRegion reg (World f t m) = World f t' m
 where
  ...
  t'    = foldl' (\hm (i, tile) -> maybeInsert i tile hm)
                 t
                 (map (id &&& loadTile) tiles)

  maybeInsert k v tm | k `M.member` tm = tm
  ...
  loadTile tc = (listArray tileBounds terrain, listArray tileBounds entities)
   where
    ...
    (terrain, entities) =
      unzip $ map (f . plusOffset tileCorner) (range tileBounds)
